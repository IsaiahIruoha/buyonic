...
productRouter.get('/slug/:slug', async (req, res) => {
  ...
  const product = await Product.findOne({ slug: req.params.slug }); //searching for a product in mongoose using its slug
  if (product) {
    res.send(product);
  } else {
...
productRouter.get('/:id', async (req, res) => {
  ...
  const product = await Product.findById(req.params.id); //searching for a product using its _id
  if (product) {
    res.send(product);
  } else {
    res.status(404).send({ message: 'Product Not Found' });
